...
# 目标线 y = 0.9x + 1.5：x=0、2、4 三点恰好在线上，其余点小幅偏离、上下交替
xs = [0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5]
ys = [1.5, 2.17, 2.15, 3.01, 3.3, 3.51, 4.45, 4.47, 5.1, 5.75, 5.82]
a, b = fit_line(xs, ys)
...
for xi, yi in zip(xs, ys):
    y_hat = a * xi + b
    ax.plot([xi, xi], [yi, y_hat], color=C_RED, lw=3.0,
            zorder=4, solid_capstyle="round",
            label="残差 $r_i$（预测−实际）" if first else None)
    first = False

ax.set_xlabel("x")
ax.set_ylabel("y")
ax.set_title("最小二乘：让所有红色“残差线段”的平方和最小\n（损失函数的祖宗：Σ(预测−实际)²）", fontsize=13)

...
> 3. 理解那句关键公式"转 mθ 再转 −nθ = 转 (m−n)θ"（旋转复合 = 角度相加）；
...
先转 nθ 度、再转 mθ 度，总效果就是转 (m+n)θ 度。**转角的复合 = 角度的相加**（Day 3 你已经亲手验证过：R(90)@R(90) = R(180)，就是"90+90=180"）。

那么"转 mθ 度"的**逆**（反方向转），就是"转 −mθ 度"。于是：

```
    R(mθ) @ R(−nθ) = R( (m−n)·θ )
```

即：**"先反着转 nθ 度，再正着转 mθ 度"，总效果 = 转 (m−n)θ 度。**

这个等式为什么重要？因为它意味着——**两个词之间的"旋转关系"，只和它们的相对位置 (m−n) 有关，和绝对位置无关。**

- 第 5 个词和第 3 个词，相对位置差 2；
- 第 100 个词和第 98 个词，相对位置也差 2；
- 这两对词的"旋转差"完全相同，都是转 2θ 度。
...
- `rotate_2d(v, theta)`：把 2 维向量旋转 θ 度；
- `apply_rope_2d(xs, theta)`：对一堆词的向量，按位置 m 各自转 mθ 度；
...
> ⚠️ **易踩坑（预告版先行警告）：** 真实的 RoPE 不像这里这么简单——它不是把整个向量转一个角度，而是**把向量两两配对成很多个 2 维小块，每块用不同的"频率" θ 去转**（高维度转得慢、低维度转得快），这样才既能编码远近、又能编码细节。别慌，这个"两两配对"的细节 Week 6 会拆成小步带你走。今天你只需要带走一个直觉：**"位置 = 转的角度"**。
...
- RoPE 的做法：位置 m 的词向量**转 mθ 度**，把位置写进"方向"；

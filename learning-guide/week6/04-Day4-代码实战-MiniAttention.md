...
2. **每行和 = 1**：右图任取一行把每格数字加起来，约等于 1.00（格子里的数四舍五入到了两位小数，加出 0.99 或 1.01 都正常；没舍入的精确值每行和恰好 = 1）。这是 softmax 的功劳。
...
3. "吃"那行的权重读右图每格数字即可，求和 ≈ 1.00（格子上是两位小数的舍入值，0.99~1.01 都算对；精确值恰好 = 1）。

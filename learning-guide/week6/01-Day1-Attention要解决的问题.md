...
3. **"把看重的信息汇总起来"怎么算子化？** —— 拿到了权重，就把每个词的向量按权重**加权求和**。加权求和是什么？就是 Week 2 的老朋友**线性组合**：w1·v1 + w2·v2 + w3·v3 + w4·v4。

看，问题本身不难，难的是**把它落成你能算的东西**。落成之后，整条流水线就清晰地浮现出来了：

```
"吃"该怎么理解 = 把"我/爱/苹果"的向量，按"谁跟吃相关"给的权重，加权求和
                = Σ (亲密度权重_i × 词_i 的向量)
```

这个 Σ 求和式，就是 Attention 的雏形。后面四天，我们要一砖一瓦把它从"口号"变成"能跑的代码"。
...
给每个词一个随机的 2 维向量（就像 Week 1 里给颜色、给词找坐标那样，只是这里是随便甩在二维平面上的 6 个点）。然后机器会算出一张 6×6 的**亲密度表**——第 i 行第 j 列，就是"第 i 个词看第 j 个词，该给多少关注"。
...
ChatGPT、Claude 这些大模型之所以"读得懂上下文"，底层就靠这个机制：**每个 token 在每一层里，都对整句里所有其它 token 算一次"亲密度"，再按权重把它们的含义吸收进自己**。你今天建立的"谁看谁、看多重"的直觉，就是那个让模型"理解语境"的引擎的第一性原理。真实的 Transformer 里这份"亲密度"会算成千上万次（多层、多头），但**每一次都长得和本周你要写的那个 Σ 一模一样**。

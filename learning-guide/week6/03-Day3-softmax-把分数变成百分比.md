...
- **差距被放大**：x 越大，e^x 大得越猛（e^10 ≈ 22026，而 e^0 = 1）。于是"分数最大的那个"会被推到绝对统治地位——正是我们要的"重要的多拿"。
...
```
softmax(scores) = [ exp(s₁)/Σexp,  exp(s₂)/Σexp,  ...,  exp(sₙ)/Σexp ]
                   其中 Σexp = exp(s₁) + exp(s₂) + ... + exp(sₙ)
```

分母 Σexp 是所有取指数之后的数的总和，作用就是"逼总分归一成 1"。
...
减最大值 1001 之后：  [-1, 0]
exp(-1) = 0.368, exp(0) = 1
归一化： [0.368/1.368, 1/1.368] ≈ [0.269, 0.731]
```
...
total = 0.3679 + 1 = 1.3679
w0 = 0.3679 / 1.3679 ≈ 0.2689
w1 = 1.0000 / 1.3679 ≈ 0.7311
检查：0.2689 + 0.7311 = 1.0000 ✓，且 w0 < w1（分数 1 < 2，权重也小）✓
```
...
- ✅ softmax 两步：exp（变正 + 拉差距）→ 除以总和（和 = 1）。
- ✅ 数值技巧：先减最大值，结果不变、只防溢出。
- ✅ 手算 [1,2] → [0.269, 0.731]，差距被指数放大。

---

## 动手练习

1. **手算**：scores = [0, 0, 3]，用纸笔按"减最大值 → exp → 归一化"三步算出三个权重，验证和为 1，并指出谁拿了大头。
...
1. 减 3 得 [-3,-3,0]，exp 得 [e^-3, e^-3, 1]，权重 ≈ [0.045, 0.045, 0.91]（第三项拿大头）。

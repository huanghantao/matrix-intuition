...
> 5. 拿到一张**直觉分工图**：知道流水线的每一站该调用哪一套旧直觉（运动 / 亲密度 / 配方）。
...
### 这个"累加"是从哪冒出来的？——按行切（Week 3 Day 3 的第二把刀）

你可能会犯嘀咕：$\mathrm{weights}\cdot V$ 明明是**矩阵乘矩阵**，怎么下一句话就变成了"一串 $V$ 的加权累加"？中间没有跳步，用的正是 Week 3 Day 3 §3.3b 那把"按行切"的刀。

$\mathrm{weights}$ 是 $n\times n$（第 $i$ 行 = 词 $i$ 的注意力配方），$V$ 是 $n\times d$（第 $k$ 行 = 词 $k$ 的干货）。按 Week 3 Day 4 的格子规则，乘积的第 $i$ 行第 $j$ 列是：

$$
\mathrm{out}[i][j] = \sum_k \mathrm{weights}[i][k]\cdot V[k][j]
$$

现在**固定 $i$ 不动**，把整行的格子拼起来、按 $k$ 归拢同类项——注意求和指标 $k$ 一边沿着 weights 的第 $i$ 行走，一边恰好穿过 $V$ 的第 $k$ 行：

$$
\mathrm{out}[i] = \sum_k \mathrm{weights}[i][k]\cdot\left(V\ \text{的第}\ k\ \text{行}\right)
$$

这正是本节开头那个式子（把 $k$ 改名 $j$ 而已）。一句话：**weights 的第 $i$ 行是配方，$V$ 的各行是原料；按行切开，每个词各调各的配方**——词 $i$ 的新向量只由它自己那一行配方决定，别的词一点也不掺和。

![weights·V 按行切：用"吃"的配方调配 V 的各行](../../figures/out/w6d2_weightsV_rows.png)

左图是矩阵视角："吃"那一行 $[0.8,\ 0.1,\ 0.1]$ 把 $V$ 的三行分别乘上 $0.8$、$0.1$、$0.1$ 再加起来，一次配出 out 的第 2 行。右图是几何视角，还藏着一个"调配"独有的性质：**因为权重和为 1，输出一定落在三个 $V$ 点围成的三角形（"包围圈"）里**——这叫**凸组合**：原料摆在那儿，怎么调都跑不出圈外。对比着记：§2.2 的 $X\cdot W_q$ 是"变换"，坐标可正可负、没有这种约束——这正是下一节"直觉分工"里它们披不同外衣的原因。
...
看到这里你可能有个"矛盾"要清算：同样是矩阵乘法，§2.2 说 $X\cdot W_q$ 要调用 Week 3 的**运动**直觉，§2.5 又说 $\mathrm{weights}\cdot V$ 要调用 Week 2 的**配方**直觉——**到底用哪个？**

答案：两个都用，**各管一站**。它们的机械骨架本来就是同一个——矩阵乘法 = **左边的一行是配方，调配右边的各行**（Week 3 Day 3 §3.3b 那把"按行切"的刀；而 Week 3 的"变换"本身，也是这台机器披上"尺子皮"的样子：配料是尺子、配比是坐标）。差别只在一件事：**右边的行是什么东西**——

- $X\cdot W_q$：右边 $W_q$ 的行是**尺子**（训练出来的 $e_i$ 新家），左边喂进来的是坐标（步数）→ 披**运动**皮。特征是"进一个、出一个"：词 $i$ 进去，词 $i$ 的新坐标出来，身份没变，只是换到查询空间重新表达。你在这里问的全是运动问题："新空间长什么样？尺子怎么摆？"
- $\mathrm{weights}\cdot V$：右边 $V$ 的行是**原料**（一个个词的干货），左边喂进来的是百分比 → 披**配方**皮。特征是"进多个、出一个"：所有词的 $V$ 进去，揉成一个新向量出来；权重和为 1，输出落在原料围成的"包围圈"里（§2.5 那张图的三角形）。你在这里问的全是配方问题："谁的份额大？混进了谁的味道？"

所以这条流水线，其实是**同一台"加权求和机器"穿着四件外衣在跑**：

![Attention 流水线四站直觉分工图](../../figures/out/w6d2_intuition_stations.png)

> 📌 **划重点：** 见到矩阵乘法，别背"该用哪个直觉"，走两步流程——
> 1. **上骨架**：左行是配方，右行被调配，第 $i$ 行配出第 $i$ 行（机械层永远如此）；
> 2. **认一次脸**：问"**右边的行是什么？**"——是尺子 → 调用 Week 3 的运动直觉；是一批内容向量 → 调用 Week 2 的配方直觉；是等着被配对的向量（如 $K^T$ 的列）→ 调用 Week 5 的亲密度直觉。
>
> 一套骨架走天下，外衣靠认脸。以后再见到陌生的矩阵乘法，这套流程照样管用。

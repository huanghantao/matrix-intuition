...
> 3. 用两条"已教事实"亲手推出那个魔法性质：**$q_m\cdot k_n$ 只依赖相对位置 $(m-n)$**；
> 4. 认识旋转的另一面：**保长度**（$|Rv| = |v|$）；
...
> **真实 RoPE 把词向量的每一对维度（第 0-1 维、第 2-3 维、第 4-5 维……）都两两一组，按不同的频率 $\theta$ 去旋转。** 于是低维组转得慢（编码长距离、宽泛的位置关系）、高维组转得快（编码短距离、精细的相邻关系），各管各的粒度。

换句话说：你 `rotate_2d` 里的"一个 $\theta$"放大成"每对维度一个不同的 $\theta$"，再在每一对上各做一次你写过的旋转。**机制没变，只是把一条链扩成好多条不同转速的链。** 真到读源码那天，你会一眼认出"这就是我 Week 6 写过的那东西"。

...
光看不练假把式——真实 RoPE 的数学你已经全部见过（5.8.1~5.8.3 的三步 + 5.8.4 的手算），现在亲手写进 `matrixlab/rope.py`。这里用纯 Python 列表 + `math` 逐对循环实现，和 Week 1~7 的风格一致；真实工程里会用 NumPy 把所有对一次算完（`reshape` 成对 + cos/sin 两张表，见 5.8.8 的工程细节），骨架一模一样。

...
把它展开，就是 Week 3 学的矩阵乘向量（行 × 列，对应相乘再相加）：

- 第一行：`1·x + 1·y = 3`，正是 `x + y = 3`；
- 第二行：`1·x + (−1)·y = 1`，正是 `x − y = 1`。
...
>
> 敏锐！完全一样。Week 4 Day 3 学的逆矩阵 `A⁻¹`，干的正是这件事：`x = A⁻¹ b`，"把 A 做过的事撤销掉，就找回原来的 x"。区别只在于：**那会儿我们承认"有逆矩阵"这个前提，用一条公式直接套**；而解方程组，是**不预设逆矩阵存在，纯靠消元硬算**。你马上会看到两条路殊途同归。
...
对这道题 `a=1, b=1, c=1, d=−1`，行列式 `det = ad − bc = 1·(−1) − 1·1 = −2`，所以
...
```
x = A⁻¹ b = [ 0.5   0.5 ] [ 3 ] = [ 0.5·3 + 0.5·1 ] = [ 2 ]
             [ 0.5  −0.5 ] [ 1 ]   [ 0.5·3 − 0.5·1 ]   [ 1 ]
```
...
**结论**：逆矩阵是"一眼看穿的公式解"，消元是"逐步碾过去的硬解"。前者的存在有个前提（行列式 ≠ 0），后者则是一条**永远能走、还能机械化推广到 n 元**的"笨办法"。所以明天的编程，我们选消元——因为它能写成循环，对任意 n 都成立。
...
2. **想一想**：上一题里 `det = 3·(−1) − 2·2 = −7`，不等于 0。如果 det 恰好是 0，几何上这两条直线是什么关系？
...
本章没有要写的代码，参考答案见明天 Day 2 的 `reference/gauss.py`。第 1 题你若用第 2 题的逆矩阵公式算，A⁻¹ = (−1/7)·[−1 −2; −2 3]，乘 b=(8,3) 得 (2,1)。

...
| 章节 | 标题 | 你会得到 |
|---|---|---|
| Day 1 | [点积——一个数如何度量两个方向的关系](./01-Day1-点积-一个数度量两个方向的关系.md) | 点积的定义与几何意义：对应相乘相加 = \|u\|\|v\|cosθ；正交（垂直）⇔ 点积为 0 |
...
| Day 5 | [AI 联系——embedding 相似度就是夹角，attention 的分数就是点积](./05-Day5-AI联系-embedding相似度就是夹角.md) | embedding 检索、attention 的 QKᵀ 分数、"国王−男人+女人≈女王"的验证思路 |
...
- 把 u "投影"到 v 上，得到一条**影子**，长度是 `|u|·cosθ`；
- 点积 `u·v` 就是这条影子再乘上 v 的长度 `|v|`；
- 夹角 θ 越小，影子越长，点积越大——**"亲密度"越高**。

这一张图，是本周五章全部内容的根。点积 → 长度 → 夹角 → 余弦相似度 → embedding 检索与 attention，都是同一件事在不同场景下的化身。
...
- "国王 − 男人 + 女人 ≈ 女王"这句 AI 圈黑话背后的数学：它等价于"用向量加法在语义空间里挪位置，再用余弦相似度收账"。

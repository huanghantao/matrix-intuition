...
- 对"**余弦相似度为什么是 embedding 的标准度量**"的透彻理解，并且**因果链不反**："相似=方向像"是人为选定的定义 → $\cos\theta$ 是数学为该定义逼出的最佳度量（与夹角单调等价、好算、逐维"意见一致度"）→ embedding 的语义恰好住在方向上（经验事实：长度被词频/语气污染）→ 于是工程上清一色选余弦。
